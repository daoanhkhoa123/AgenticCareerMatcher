...
def match_jobs_by_embedding(profile: CVProfile, preferences: str | None, limit: int) -> dict:
    logger.info("Matching CV profile (%d skill(s)) by embedding (preferences=%r, limit=%d)", len(profile.skills), preferences, limit)
    ...
    with get_session() as session:
        ...
        logger.info("Found %d embedding match(es) out of %d job(s) in database", len(matches), total_jobs_in_db)
